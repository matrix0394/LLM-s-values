#!/usr/bin/env python3
"""
Generate Fig. S3: Model and Response Analysis figures.

Output: SI/figures/FigS3/

Figures:
- S3A: Consistency distribution by model (baseline vs roleplay)
- S3B: Response distribution by question
- S3C: Imitation shift by model
- S3D: Imitation country shift summary (top/bottom 15)
...
from si_config import (
    ...
    save_figure, MODEL_COLORS, set_publication_style
)
...
# =============================================================================
# Unified Color Palette for Fig. S3 figures (same as S5)
# =============================================================================
...
# Two-color figures (S3A, S3B, S3C, S3D, S3F): Red + Green
# Three-color figures (S3E, S3G): Red + Green + Blue

# S3A: Baseline vs Roleplay
CONDITION_COLORS = {
    'Baseline': COLOR_BLUE,   # Blue
    'Roleplay': COLOR_RED,    # Red
}

# S3B, S3C, S3D: Good vs Bad performance
PERFORMANCE_COLORS = {
    'good': COLOR_GREEN,      # Green (good/low distance)
    'bad': COLOR_RED,         # Red (bad/high distance)
}

# S3E: Model size (2 categories)
SIZE_COLORS = {
    'Small': COLOR_RED,       # Red
    'Large': COLOR_GREEN,     # Green
}

# S3F: Open vs Closed source
SOURCE_COLORS = {
    'Open Source': COLOR_GREEN,      # Green
    'Closed Source': COLOR_RED,      # Red
}

# S3G: Model origin (3 categories)
ORIGIN_COLORS = {
    'USA': COLOR_BLUE,            # Blue
    'China': COLOR_RED,           # Red
...
def load_and_preprocess_data():
    """Load baseline and roleplay PCA data from project data (not old SI)."""
    from si_config import load_ivs_data, load_baseline_data, load_roleplay_data
...
def generate_s3a_consistency_distribution(baseline_df, roleplay_df, output_dir):
    """Generate Fig. S3A: Consistency distribution by model."""
    print("Generating FigS3A: Consistency distribution by model...")
    ...
    plt.tight_layout()
    save_figure(fig, output_dir / 'FigS3A_consistency_distribution')
    print("  ✅ Saved: FigS3A_consistency_distribution")


def generate_s3b_response_distribution(baseline_df, roleplay_df, output_dir):
    """Generate Fig. S3B: Model flexibility analysis - deviation from baseline."""
    print("Generating FigS3B: Model flexibility analysis...")
    ...
    plt.tight_layout()
    save_figure(fig, output_dir / 'FigS3B_model_flexibility')
    print("  ✅ Saved: FigS3B_model_flexibility")


def generate_s3c_imitation_shift_by_model(roleplay_df, ivs_df, output_dir):
    """Generate Fig. S3C: Imitation shift by model (best performers on top)."""
    print("Generating FigS3C: Imitation shift by model...")
    ...
    ax.set_title('Cultural Imitation Accuracy by Model',
                fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    save_figure(fig, output_dir / 'FigS3C_imitation_accuracy_ranking')
    print("  ✅ Saved: FigS3C_imitation_accuracy_ranking")


def generate_s3d_country_shift_summary(roleplay_df, ivs_df, output_dir):
    """Generate Fig. S3D: Country shift summary (top/bottom 15)."""
    print("Generating FigS3D: Country shift summary...")
    ...
    save_figure(fig, output_dir / 'FigS3D_country_imitation_ranking')
    print("  ✅ Saved: FigS3D_country_imitation_ranking")


def generate_s3e_model_size_analysis(roleplay_df, ivs_df, output_dir):
    """Generate Fig. S3E: Model size vs accuracy - Small vs Large comparison."""
    print("Generating FigS3E: Model size analysis...")
    ...
    fig.suptitle('Model Size vs. Imitation Accuracy', fontsize=13, fontweight='bold', y=0.98)
    plt.subplots_adjust(top=0.88, bottom=0.12, left=0.08, right=0.95, wspace=0.3)
    save_figure(fig, output_dir / 'FigS3E_model_size_analysis')
    print("  ✅ Saved: FigS3E_model_size_analysis")


def generate_s3f_open_vs_closed(roleplay_df, ivs_df, output_dir):
    """Generate Fig. S3F: Open vs closed source comparison - clean bar chart."""
    print("Generating FigS3F: Open vs closed source analysis...")
    from scipy import stats
    ...
    fig.suptitle('Open-Source vs. Closed-Source Comparison', fontsize=13, fontweight='bold', y=0.98)
    plt.subplots_adjust(top=0.88, bottom=0.12, left=0.08, right=0.95, wspace=0.3)
    save_figure(fig, output_dir / 'FigS3F_open_vs_closed_source')
    print("  ✅ Saved: FigS3F_open_vs_closed_source")


def generate_s3g_vendor_analysis(roleplay_df, ivs_df, output_dir):
    """Generate Fig. S3G: Model origin (USA/China/Europe) comparison - clean bar chart."""
    print("Generating FigS3G: Model origin analysis...")
    from scipy import stats
    ...
    fig.suptitle('Model Origin and Vendor Analysis', fontsize=13, fontweight='bold', y=0.98)
    plt.subplots_adjust(top=0.88, bottom=0.10, left=0.08, right=0.95, wspace=0.3)
    save_figure(fig, output_dir / 'FigS3G_vendor_origin_analysis')
    print("  ✅ Saved: FigS3G_vendor_origin_analysis")


def main():
    """Main function to generate all Fig. S3 figures."""
    print("=" * 60)
    print("Generating Fig. S3: Model and Response Analysis")
    print("=" * 60)
    
    # Output directory
    output_dir = FIGURES_DIR / 'FigS3'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Load data
    baseline_df, roleplay_df, ivs_df = load_and_preprocess_data()
    
    # Generate all Fig. S3 figures
    generate_s3a_consistency_distribution(baseline_df, roleplay_df, output_dir)
    generate_s3b_response_distribution(baseline_df, roleplay_df, output_dir)  # Now: flexibility analysis
    generate_s3c_imitation_shift_by_model(roleplay_df, ivs_df, output_dir)
    generate_s3d_country_shift_summary(roleplay_df, ivs_df, output_dir)
    generate_s3e_model_size_analysis(roleplay_df, ivs_df, output_dir)
    generate_s3f_open_vs_closed(roleplay_df, ivs_df, output_dir)
    generate_s3g_vendor_analysis(roleplay_df, ivs_df, output_dir)
    
    print("\n" + "=" * 60)
    print(f"✅ All S3 figures saved to: {output_dir}")
    print("   Generated figures:")
    print("   - FigS3A: Consistency distribution (baseline vs roleplay)")
    print("   - FigS3B: Model flexibility (deviation from baseline)")
    print("   - FigS3C: Model ranking by imitation accuracy")
    print("   - FigS3D: Country difficulty ranking")
    print("   - FigS3E: Model size vs accuracy")
    print("   - FigS3F: Open vs closed source comparison")
    print("   - FigS3G: Model origin and vendor analysis")
    print("=" * 60)
