...
脚本优先读取项目数据，SI/pca 为 fallback：
- LLM roleplay PCA: `data/llm_pca/multilingual/Table_S7_*.csv` 或 `SI/pca/Table_S7_*.csv`
- IVS coordinates: `data/country_values/Table_S5_*.csv` 或 `SI/pca/Table_S5_*.csv`
- `results/analysis/stage0_vs_stage3/english_advantage_average.csv` - 预计算英语优势（由 `analysis/core/stage0_vs_stage3_distance.py` 生成）

### Optional Data (for regression analysis)

回归用外部数据位于 `data/external/`：
- GDP: `data/external/world_bank_gdp/`
- HDI: `data/external/HDR25_Statistical_Annex_HDI_Table.xlsx`
- 殖民历史: `data/external/external_colonial_history.csv`
...
需先运行主分析脚本生成 stage0 vs stage3 数据：
```bash
# 从项目根目录运行
python3 analysis/core/stage0_vs_stage3_distance.py
```
