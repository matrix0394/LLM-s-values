...
class RoleplayMultilingualAnalysisRunner:
            ...
            for model in models:
                ...
                for lang in languages:
                    ...
                    if len(model_lang_data) > 0:
                        ...
                        # 检测重复坐标并添加偏移
                        coords_rounded = coords_data.round(6)
                        duplicates = coords_rounded.duplicated(keep=False)
                        ...
                        if duplicates.any():
                            ...
                            for idx in coords_data[duplicates].index:
                                # 添加小的随机偏移（0.01-0.03范围内）
    ...
    def _run_comprehensive_test_with_full_pipeline(self):
        """运行全面测试并完成整个分析流程"""
        try:
            print("🚀 开始全面测试 (7模型×12国家×4语言×5重复)")
            
            # 估算成本和时间
            total_calls = 7 * 12 * 4 * 57 * 5  # 模型×国家×语言×问题×重复
            estimated_cost = total_calls * 0.0003  # 估算每次调用成本（更新后的模型）
            estimated_minutes = total_calls * 2.0 / 60  # 估算时间（秒转分钟，考虑新模型）
    ...
    def _run_small_scale_test_with_full_pipeline(self):
        ...
        try:
            ...
            # 估算成本和时间
            total_calls = 3 * 3 * 4 * 57 * 3  # 模型×国家×语言×问题×重复
            estimated_cost = total_calls * 0.0003  # 估算每次调用成本（更新后的模型）
            estimated_minutes = total_calls * 2.0 / 60  # 估算时间（秒转分钟，考虑新模型）
    ...
    def _run_multilingual_interview_with_config(self, config_type: str):
        ...
        try:
            ...
            if config_type == "comprehensive":
                countries = ["China", "United States", "Germany", "Japan", "India", "Brazil", 
                           "Russia", "France", "United Kingdom", "South Korea", "Australia", "Canada"]
            ...
            languages = ["zh", "en", "es", "ar"]  # 中文、英文、西班牙语、阿拉伯语
...
def main():
    ...
    print("请选择运行模式:")
    print("  1. 全面测试 (7模型×12国家×4语言×5重复) - 完整流程")
