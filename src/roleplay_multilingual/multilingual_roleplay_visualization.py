...
class MultilingualRoleplayVisualizer(BaseCulturalMapVisualizer):
    ...
    def __init__(self, data_path: str = "../data", results_path: str = "../results"):
        ...
        self.viz_dir = self.results_path / "multilingual_visualizations"
    ...
    def load_pca_results(self, results_file: str = None) -> pd.DataFrame:
        ...
        if results_file is None:
            # 查找最新的PCA结果文件
            result_files = list(self.results_dir.glob("multilingual_pca_results_*.pkl"))
            if not result_files:
                raise FileNotFoundError("未找到多语言PCA结果文件")
    ...
    def load_analysis_results(self, analysis_file: str = None) -> Dict:
        ...
        if analysis_file is None:
            # 查找最新的分析结果文件
            analysis_files = list(self.results_dir.glob("multilingual_analysis_results_*.json"))
    ...
    def create_interactive_cultural_map(self, df: pd.DataFrame, suffix: str = None) -> str:
        ...
        for language in valid_df['language'].unique():
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='markers+text',
                name=language,
                text=lang_data['country'],
                textposition='top center',
                marker=dict(
                    size=12,
                    color=self.language_colors.get(language, 'gray'),
                    opacity=0.7,
                ...
                customdata=lang_data[['model']].values
            ))
        
        fig.update_layout(
            title='多语言角色扮演文化地图',
            xaxis_title='PC1 (传统 vs 世俗理性价值观)',
            yaxis_title='PC2 (生存 vs 自我表达价值观)',
            width=900,
            height=700,
            showlegend=True,
            hovermode='closest'
        ...
        # 保存交互式图表
        output_file = self.viz_dir / f'interactive_multilingual_cultural_map_{suffix}.html'
        fig.write_html(str(output_file))
        
        print(f"交互式文化地图已保存: {output_file}")
    ...
    def create_language_comparison_dashboard(self, df: pd.DataFrame, analysis_results: Dict, suffix: str = None) -> str:
        ...
        # 保存仪表板
        output_file = self.viz_dir / f'multilingual_comparison_dashboard_{suffix}.html'
        fig.write_html(str(output_file))
    ...
    def create_model_performance_analysis(self, df: pd.DataFrame, suffix: str = None) -> str:
        ...
        # 保存分析图
        output_file = self.viz_dir / f'model_performance_analysis_{suffix}.html'
        fig.write_html(str(output_file))
    ...
    def create_static_summary_plots(self, df: pd.DataFrame, analysis_results: Dict, suffix: str = None):
        ...
        plt.tight_layout()
        plt.savefig(self.viz_dir / f'multilingual_summary_{suffix}.png',
    ...
    def generate_analysis_report(self, df: pd.DataFrame, analysis_results: Dict, suffix: str = None) -> str:
        ...
        # 保存报告
        report_file = self.viz_dir / f'multilingual_analysis_report_{suffix}.json'
    ...
    def create_complete_visualization_suite(self, results_file: str = None, analysis_file: str = None) -> Dict:
        ...
        return {
            ...
            'model_analysis': model_analysis,
            'analysis_report': analysis_report,
            'viz_directory': str(self.viz_dir)
        }
    ...
    def _create_visualization_index(self, suffix: str, file_paths: Dict) -> str:
        ...
        index_file = self.viz_dir / f'multilingual_visualization_index_{suffix}.html'
