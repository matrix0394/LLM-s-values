...
REMEMBER: Numbers ONLY. Any other output = FAILURE."""
                ...
                try:
                    ...
                    if "deepseek" in model_name.lower():
                        ...
                        if hasattr(message, 'reasoning_content') and message.reasoning_content:
                            print(f"发现reasoning_content: {message.reasoning_content}")
                            ...
                            if not content.strip():
                                ...
                                if numbers:
                                    # 使用最后出现的数字序列作为答案
                                    content = numbers[-1]
                                    print(f"从reasoning中提取的答案: {content}")
                    else:
    ...
    def save_results(self, results: Dict[str, List[LLMResponse]], output_dir: str = None):
        ...
        for model_name, responses in results.items():
            ...
            # 保存pkl文件
            pkl_path = os.path.join(output_dir, f"{safe_model_name}_responses_improved.pkl")
            df.to_pickle(pkl_path)
            
            # 保存json文件（便于人工查看）
            json_path = os.path.join(output_dir, f"{safe_model_name}_responses_improved.json")
        ...
        if all_data:
            ...
            # 保存汇总pkl
            summary_pkl_path = os.path.join(output_dir, "all_models_responses_improved.pkl")
            all_df.to_pickle(summary_pkl_path)
            
            # 保存汇总json
            summary_json_path = os.path.join(output_dir, "all_models_responses_improved.json")
