...
from .llm_questionnaire import IVSQuestions, LLMResponse
...
REMEMBER: Numbers ONLY. Any other output = FAILURE."""
    ...
    def call_model_api(self, model_name: str, question_id: str, question_text: str) -> Optional[str]:
        ...
        try:
            ...
            # 为所有问题添加强制格式提示（不包含具体数字示例）
            format_hint = "\n\nOUTPUT FORMAT: "
            if question_id == "Y002":
                format_hint += "TWO NUMBERS WITH ONE SPACE"
                if "qwq" in model_name.lower():
                    format_hint += "\nYOU MUST OUTPUT EXACTLY TWO NUMBERS SEPARATED BY SPACE"
                    format_hint += "\nDO NOT SAY ANYTHING ELSE. JUST TWO NUMBERS."
            elif question_id == "Y003":
                format_hint += "1-5 NUMBERS WITH SPACES"
            else:
                format_hint += "ONE NUMBER ONLY"
    ...
    def interview_model(self, model_name: str) -> List[LLMResponse]:
        ...
        for i, question_id in enumerate(question_ids, 1):
            ...
            # 根据模型类型调整问题间延迟
            if any(x in model_name.lower() for x in ['gpt', 'claude']):
                time.sleep(0.2)  # 快速模型，短延迟
            else:
                time.sleep(0.3)  # 其他模型，稍长延迟
