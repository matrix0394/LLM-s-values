...
class ResponseValidator:
    ...
    def validate_response(question_id: str, response: str) -> tuple[bool, Any, str]:
        ...
        try:
            if question_id in ['A008', 'A165', 'E018', 'E025']:
                # 单选题：1-4
                value = int(response)
                if 1 <= value <= 4:
                    return True, value, ""
            ...
            elif question_id in ['F063', 'F118', 'F120', 'G006']:
                # 单选题：1-10
                value = int(response)
                if 1 <= value <= 10:
                    return True, value, ""
                ...
                for value in values:
                    if not (1 <= value <= 6):
                        return False, None, f"Y002选项必须在1-6之间，得到: {value}"
                ...
                for value in values:
                    if not (1 <= value <= 10):
                        return False, None, f"Y003选项必须在1-10之间，得到: {value}"
