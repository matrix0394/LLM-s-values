...
Following established practice, ten survey items that load strongly on the two dimensions are used: happiness (A008), interpersonal trust (A165), respect for authority (E018), political action (E025), importance of God (F063), attitudes toward homosexuality (F118) and abortion (F120), national pride (G006), post-materialism (Y002), and autonomy (Y003). These items define a fixed cultural coordinate system estimated from IVS data. Dimensional axes are derived once from the IVS responses, and all model-generated responses are subsequently projected into this predefined space rather than used to re-estimate the dimensions.
